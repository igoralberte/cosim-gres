...
-- DIST FUNCTION
CREATE OR REPLACE FUNCTION get_feature(tblname1 VARCHAR(60), colname1 VARCHAR(60), fem_instance VARCHAR(60), colname1_id BIGINT) RETURNS BYTEA AS
$$
DECLARE 
v_feature BYTEA;
BEGIN
...
CREATE OR REPLACE FUNCTION fem_extraction(tblname1 VARCHAR(60), colname1 VARCHAR(60), cdd VARCHAR(60), complex_data BYTEA) RETURNS BYTEA AS
$$
DECLARE 
    v_fem VARCHAR(60);
...
CREATE OR REPLACE FUNCTION dist(tblname1 VARCHAR(60), colname1 VARCHAR(60), cdd VARCHAR(80), colname1_id BIGINT, feature2 BYTEA) RETURNS FLOAT8 AS
$$
DECLARE 
    v_query_cdd TEXT;
    v_df VARCHAR(60);
...
DECLARE
 ...
 BEGIN
  ...
  SELECT lower(data_type) 
   ...
   IF v_attribute_type <> lower('BYTEA') THEN
        RAISE EXCEPTION 'Column "%" must be BYTEA type in the relation "%".', ATTRIBUTE, RELATION;
   END IF;
   ...
   IF NOT EXISTS (SELECT 1
	...
	EXECUTE 'ALTER TABLE ' || RELATION ||' ADD COLUMN ' || ATTRIBUTE ||' BYTEA';
   --ELSE
   ...
   IF v_exists_complex_metatable_column IS NULL THEN 
	EXECUTE 'ALTER TABLE ' || v_complex_metatable ||' ADD COLUMN ' || v_fem_instance_name ||' BYTEA';
   END IF;
